...
public class ControleExcecoes  extends ResponseEntityExceptionHandler{
	...
	/*tratamento da maioia dos erros*/
	@Override
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
		...
		if(ex instanceof MethodArgumentNotValidException) {
			...
			for(ObjectError objectError : list) {
				...
				System.out.println("erro 1" + msg);
			}
		}else {
			msg = ex.getMessage();
			System.out.println("outro tipo de erro" + msg);
		}
		ObjetoError objetoError = new ObjetoError();
		objetoError.setError(msg);
		objetoError.setCode(status.value() + " ==> " + status.getReasonPhrase());
		System.out.println("objetoErro" + objetoError.getCode() + objetoError.getError());
		return new ResponseEntity<>(objetoError, headers, status);
	}
	
	/*tratamento da maioia dos erros a nivel de banco de dados*/
